...
public class ListAdapterController extends ArrayAdapter<ThreadModel>
{
	 ...
	 * to the MainActivity. Much of this was taken from
	 * https://github.com/zjullion
	 * /PicPosterComplete/blob/master/src/ca/ualberta/
	 * cs/picposter/view/PicPostModelAdapter.java
	 * 
	 * @author emar
	 * @param c
	 * @param resource
	...
	public ListAdapterController(Context c, int resource, int resourceId,
			LinkedList<ThreadModel> topics)
	{
	...
	public void addThread(ThreadListModel topics, String comment,
			UserModel user, Location location, String title, Bitmap image)
	{

		if (title == null && image == null) // not a top level comment and no
											// image
		{
		...
		else if (title == null && image != null) // not a top level comment but
													// image exists
		{
		...
		else if (title != null && image != null) // top level comment, image
													// attached
		{
			ThreadModel tm = new ThreadModel(comment, image, user, location,
					title);
