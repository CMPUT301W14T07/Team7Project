...
public class TopicCreationTest extends
	...
	public void testTopicCreate() {
		...
		assertEquals("Author name isc correct", "Ash Ketchum", topic.getAuthorName());
		assertEquals("Unique author name is the same as the users", topic.getAuthorUnique(), user.getUniqueID());
	}
	...
	public void testThreadsInsertedProperly() {
		...
		ThreadModel threadTwo = new ThreadModel("Caught Charmander", null, user.getName(), user.getUniqueID());
		topic.setLastThreadUniqueID();
		threadTwo.setUniqueID(topic.getLastThreadUniqueID());
		topic.addThread(threadTwo);
		
		ThreadModel threadThree = new ThreadModel("Caught Bulbasaur", null, user.getName(), user.getUniqueID());
		topic.setLastThreadUniqueID();
		...
		assertEquals("Threads have been correctly entered", pokedexThreads, topic.getThreads());
	}
