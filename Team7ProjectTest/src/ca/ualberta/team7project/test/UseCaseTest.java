 ...
 */
/*
 * public void editCommentTest() { // Initial test is the same as
 * addTopLevelCommentTest() TopicModel topic = new TopicModel(); ThreadModel
 * thread = new ThreadModel(); UserModel user = new UserModel();
 * 
 * // TODO Maybe have a helper class so we aren't creating a new user for each
 * Test. user.setName("Bob");
 * 
 * thread.setComment("This is my comment"); thread.setLocation();
 * thread.setAuthor(CommentAuthor author = new CommentAuthor(user));
 * topic.addThread(thread);
 * 
 * assertTrue("Comment is top level for edit comment",
 * topic.threads.contains(thread));
 * 
 * thread = topic.getThreads().getLast(); thread.setComment("Updated comment");
 * 
 * assertEquals("Edited thread should represent the new comment",
 * "updated comment", topic.getThreads().getLast().getComment());
 * 
 * } }
 */